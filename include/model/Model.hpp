...
class Model {
    ...
    std::list<DroppedHead>  dropped_heads_;
    std::list<PhantomGecko> phantom_geckos_;
...
public:
    ...
    const std::list<DroppedHead>&  getDroppedHeads() const { return dropped_heads_; }
...
private:
    ...
    void handleDroppedHeadCollisions();

    void dropHead(Snake& snake, std::vector<std::pair<int, ControlledBy>>& respawns);
    void respawnSnake(int color, ControlledBy ctrl);
